...
describe('ContextAssembler (Agent Runtime 拆分)', () => {
  function create(ragResults: any[] = []) {
    const ragService = { searchSimilarity: jest.fn().mockResolvedValue(ragResults) };
  ...
  it('为缺失 parts 的消息补 text part（AI SDK 兼容）', () => {
    ...
    const messages = assembler.sanitizeMessages([
      ...
      { role: 'assistant', content: '嗨', parts: [{ type: 'text', text: '嗨' }] },
      null,
    ]);
  ...
  it('知识模式下在最后一条用户消息前注入 RAG 背景', async () => {
    ...
    const messages = assembler.sanitizeMessages([{ role: 'user', content: '转账限额多少？' }]);
    const { injected } = await assembler.injectRagContext(messages, { userId: 'u1', source: 'web', userMessage: '转账限额多少？' }, 'knowledge');
  ...
  it('非搜索/知识模式不注入 RAG', async () => {
    const assembler = create();
    const messages = assembler.sanitizeMessages([{ role: 'user', content: 'x' }]);
    const { injected } = await assembler.injectRagContext(messages, { userId: 'u1', source: 'web', userMessage: 'x' }, 'none');
