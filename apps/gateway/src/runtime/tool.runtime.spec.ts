...
describe('ToolRuntime (Agent Runtime 拆分)', () => {
  function create(overrides: Record<string, any> = {}) {
    const deps = {
      zentaoService: { getBugInfo: jest.fn(), searchBugs: jest.fn(), resolveBug: jest.fn() },
      rpcGateway: { sendToCli: jest.fn() },
      ragService: { searchSimilarity: jest.fn().mockResolvedValue([]) },
      tracingService: {
        traceCall: jest.fn(async (_n: string, _a: unknown, fn: (span: any) => Promise<any>) =>
          fn({ setAttribute: jest.fn() }),
        ),
      },
      skillLoader: { activate: jest.fn(), getSkill: jest.fn(), buildCatalogXml: jest.fn(), loadAiguide: jest.fn() },
      interactiveManager: { getClarifyTool: jest.fn(() => ({ agp_intent_clarify: 'clarify' })) },
      mcpManager: { getAITools: jest.fn().mockResolvedValue({ mcp_alpha: 'mcp-alpha' }) },
      policyEvaluator: {
  ...
  it('无会话模式：原子工具 + 澄清工具 + MCP 工具直接合并，不做审批包装', async () => {
    ...
    const tools = await runtime.buildTools({ userId: 'u1', source: 'web', userMessage: 'hi' });
  ...
  it('会话模式：MCP 工具按策略判定走审批包装', async () => {
    const { runtime, deps } = create();
    const tools = await runtime.buildTools({ userId: 'u1', source: 'web', userMessage: 'hi' }, 'sess_1');
    expect(deps.policyEvaluator.shouldWrap).toHaveBeenCalledWith('mcp_alpha', true);
  ...
  it('activate_skill 委托 SkillLoader 加载技能', async () => {
    ...
    const tools = await runtime.buildTools({ userId: 'u1', source: 'web', userMessage: 'hi' });
    const result = await tools.activate_skill.execute({ skill_name: 'banking' });
    expect(deps.skillLoader.activate).toHaveBeenCalledWith('banking');
    expect(result).toMatchObject({ message: 'Skill "banking" activated.', skill_content: 'SKILL_CONTENT' });
  });
