...
describe('WorkService (Phase 6 6d Work 投影)', () => {
  ...
  function create(overrides: any = {}) {
    const prisma = {
      workProject: {
        ...
        findMany: jest
          .fn()
          .mockResolvedValue([{ id: 'p1', name: 'Ocean v2', tasks: [] }]),
        create: jest
          .fn()
          .mockResolvedValue({ id: 'p1', name: 'Ocean v2', spaceId: 'work' }),
        findFirst:
      ...
      workTask: {
        ...
        create: jest
          .fn()
          .mockResolvedValue({ id: 't1', title: 'task', spaceId: 'work' }),
        findFirst:
          overrides.taskFindFirst ?? jest.fn().mockResolvedValue({ id: 't1' }),
  ...
  it('createProject：owner=调用者 + 强制归属 Work Space', async () => {
    ...
    expect((svc as any).prisma.workProject.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ ownerId: 'u1', spaceId: 'work' }),
      }),
    );
  });
  ...
  it('项目不存在时 createTask 抛 NotFound', async () => {
    ...
    await expect(
      svc.createTask('u1', 'nope', { title: 't' }),
    ).rejects.toBeInstanceOf(NotFoundException);
  });

  it('updateTask：状态流转 + 改派', async () => {
    const svc = create();
    const task = await svc.updateTask('u1', 't1', {
      status: 'done',
      assigneeId: 'u2',
    });
  ...
  it('无 Membership 用户 → Forbidden', async () => {
    ...
    await expect(svc.listProjects('outsider')).rejects.toBeInstanceOf(
      ForbiddenException,
    );
  });
});
