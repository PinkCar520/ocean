...
export class OrchestratorService {
  ...
  async syncPlan(sessionId: string, tasks: any[]) {
    console.log(
      `[Orchestrator] Syncing ${tasks.length} tasks for session ${sessionId}`,
    );
    ...
    this.server
      .to(sessionId)
      .emit('workflow_plan_synced', { sessionId, tasks });
  }
  ...
  async updateTaskProgress(
    ...
    console.log(
      `[Orchestrator] Task ${taskId} updated to ${status} in session ${sessionId}`,
    );
    ...
    if (targetTask) {
      await this.prisma.task.update({
        ...
        data: {
          status,
          metadata: metadata
            ? { ...((targetTask.metadata as any) || {}), ...metadata }
            : (targetTask.metadata as any),
          updatedAt: new Date(),
        },
      });
    }
    ...
    this.server
      .to(sessionId)
      .emit('workflow_task_updated', { sessionId, taskId, status, metadata });
  }
