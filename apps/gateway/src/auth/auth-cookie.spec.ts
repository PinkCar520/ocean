...
describe('readCookie', () => {
  it('reads the Ocean session from a cookie header', () => {
    expect(readCookie(`theme=dark; ${OCEAN_SESSION_COOKIE}=header.payload.signature`, OCEAN_SESSION_COOKIE))
  ...
  it('returns undefined for a missing or malformed cookie', () => {
    ...
    expect(readCookie('theme=dark; malformed', OCEAN_SESSION_COOKIE)).toBeUndefined();
  });
});
