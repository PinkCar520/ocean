...
describe('env.validation (Phase 7 production config fail-fast)', () => {
  const base = {
    NODE_ENV: 'production',
    DATABASE_URL:
      'postgresql://app:realpass@pg.internal:5432/ocean?schema=public',
  ...
  it('rejects missing DATABASE_URL', () => {
    ...
    expect(
      v.some((x) => x.key === 'DATABASE_URL' && x.reason === 'missing'),
    ).toBe(true);
  });
  ...
  it('rejects default-credential DSN to a non-local host', () => {
    const v = validateProductionConfig({
      ...base,
      DATABASE_URL:
        'postgresql://postgres:postgres@pg.internal:5432/ocean?schema=public',
    });
    expect(
      v.some(
        (x) =>
          x.key === 'DATABASE_URL' && x.reason.includes('default-credential'),
      ),
    ).toBe(true);
  });
  ...
  it('rejects known weak JWT secret', () => {
    const v = validateProductionConfig({
      ...base,
      JWT_SECRET: 'ocean-secret-key-2024',
    });
    expect(
      v.some(
        (x) =>
          x.key === 'JWT_SECRET' && x.reason.includes('known default/weak'),
      ),
    ).toBe(true);
  });
  ...
  it('rejects short JWT secret', () => {
    ...
    expect(
      v.some((x) => x.key === 'JWT_SECRET' && x.reason.includes('too short')),
    ).toBe(true);
  });
  ...
  it('rejects missing AI provider key for the default provider', () => {
    ...
    expect(
      v.some(
        (x) =>
          x.key === 'DASHSCOPE_API_KEY' &&
          x.reason.includes('required by DEFAULT_AI_PROVIDER'),
      ),
    ).toBe(true);
  });
