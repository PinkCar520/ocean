...
export function validateProductionConfig(
  ...
  if (!dbUrl) {
    ...
    violations.push({
      key: 'DATABASE_URL',
      reason: 'looks like a default-credential DSN to a non-local host',
    });
  }
  ...
  if (!jwt) {
    ...
    violations.push({
      key: 'JWT_SECRET',
      reason: 'uses a known default/weak value',
    });
  } else if (jwt.length < 32) {
    violations.push({
      key: 'JWT_SECRET',
      reason: `too short (${jwt.length} chars, need >= 32)`,
    });
  }
  ...
  const keyEnv =
    AI_PROVIDER_KEY_ENVS[provider] ?? AI_PROVIDER_KEY_ENVS.dashscope;
  if (keyEnv && !(env[keyEnv] ?? '')) {
    violations.push({
      key: keyEnv,
      reason: `missing (required by DEFAULT_AI_PROVIDER=${provider})`,
    });
  }
