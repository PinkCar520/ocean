...
describe('SpaceService (Phase 5 数据边界)', () => {
    ...
    const prisma = {
      space: {
        ...
        findMany: jest.fn().mockResolvedValue([
          { id: 'work', slug: 'work', name: '工作空间', type: 'work', icon: null, memberships: [{ role: 'owner' }] },
        ]),
      },
      membership: {
  ...
  it('ensureLifeSpace：已存在时返回并补齐 Membership', async () => {
    ...
    const svc = create({
      spaceFindUnique: jest.fn().mockResolvedValue({ id: 'life-u1', type: 'life' }),
      membershipFindUnique: membership,
    });
    await expect(svc.ensureLifeSpace('u1')).resolves.toEqual({ id: 'life-u1', type: 'life' });
