...
export function AllChatsManager({
  ...
  return (
    <div className="flex-1 flex flex-col items-center bg-muted overflow-y-auto px-4 sm:px-8 pt-12 pb-24 relative">
        ...
        <div className="mt-8 flex flex-col gap-4">
          <div className="relative w-full sm:max-w-2xl">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2.5 bg-card border border-border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all text-foreground"
            />
            {searchQuery && (
          ...
          <div className="flex flex-wrap items-center gap-3">
             <button
                ...
                className="flex items-center gap-2 px-3 py-1.5 border border-border bg-card rounded-lg text-sm font-medium text-muted-foreground hover:bg-[#eeece9] transition-colors"
              ...
              {selectedIds.size > 0 && (
                <>
                  ...
                  <button 
                    ...
                    className="px-3 py-1.5 rounded-lg text-sm font-medium text-muted-foreground hover:bg-border/50 transition-colors"
                  >
                ...
                >
                  ...
                  >
                    {selectedIds.has(chat.id) ? (
                      ...
                      <Square className="w-5 h-5 text-[#C4C0BE] group-hover:text-muted-foreground transition-colors" />
                    )}
                  </div>
                  <div className="flex-1 py-4 px-2 min-w-0">
                    <h3 className="text-[15px] font-semibold text-foreground truncate pr-4">
                      {chat.title}
                    </h3>
      ...
      <Dialog open={isConfirmOpen} onOpenChange={setIsConfirmOpen}>
        <DialogContent className="sm:max-w-[460px] sm:rounded-[16px] gap-0 p-6">
          <DialogHeader>
            <DialogTitle className="text-2xl font-bold text-foreground">{t('all_chats.confirm_delete_title', '确认清除历史记录')}</DialogTitle>
          ...
          <DialogFooter className="mt-10 flex sm:justify-end gap-3">
            <button
              ...
              className="px-8 py-2.5 rounded-[12px] border border-border text-sm font-bold text-muted-foreground hover:bg-[#F1EFEB] transition-colors min-w-[100px]"
            >
              {t('common.cancel')}
            </button>
            <button
