          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="bg-foreground text-white px-4 py-2 rounded-lg text-sm font-semibold flex items-center gap-2 shadow-md hover:bg-foreground/80 transition-all"
              >
          ...
          {isLoading ? (
            ...
            filteredProjects.map((project) => (
              <motion.div
                layoutId={project.id}
                key={project.id}
                className="bg-card rounded-[20px] shadow-[0_4px_20px_rgba(0,0,0,0.03)] border border-transparent hover:border-primary/30 hover:shadow-[0_10px_30px_rgba(236,91,20,0.05)] transition-all overflow-hidden group cursor-pointer relative"
              >
                ...
                <div className="absolute top-3 left-3 z-20 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button 
                    ...
                    className="bg-card shadow-xl text-red-500 p-2 rounded-xl border border-red-50 hover:bg-red-50 transition-all active:scale-95"
                    title="删除项目"
                  >
                ...
                <div className="p-6 pt-2">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-[10px] text-muted-foreground font-medium ml-auto">{new Date(project.updatedAt).toLocaleDateString()}</span>
                  </div>
                  <h4 className="text-lg font-display font-bold mb-1 group-hover:text-primary transition-colors">{project.name}</h4>
                  <p className="text-muted-foreground text-sm mb-4 line-clamp-2 min-h-[40px]">{project.description?.replace(/\[CLOUD\]|\(path:.*?\)/g, '')}</p>
                  <div className="flex items-center justify-between pt-4 border-t border-[#F6F3F2]">
                    ...
                    <div className="flex items-center gap-1.5">
                      <span className="text-[10px] font-black text-foreground">{project._count?.documents || 0}</span>
