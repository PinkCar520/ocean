...
export function PipelineCard({
  ...
  const statusStyles: Record<string, StatusStyle> = {
    ...
    paused: { text: 'text-muted-foreground', bg: 'bg-[#716B67]/5', border: 'border-[#716B67]/20', icon: Clock },
  };
  ...
  return (
    <div className="bg-card border border-border rounded-[20px] overflow-hidden shadow-[0_8px_30px_rgba(0,0,0,0.04)] mt-4 max-w-[520px]">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-mono bg-muted px-2 py-0.5 rounded-md text-muted-foreground">#{id}</span>
              </div>
        ...
        {steps.map((step: UIStep, i: number) => (
          <div key={i} className="flex items-center gap-4">
            ...
            <div className={cn(
              ...
              step.status === 'waiting' && "border-border bg-card"
            )}>
              ...
              {step.status === 'waiting' && <div className="w-2 h-2 rounded-full bg-border" />}
            </div>
            {/* Step name */}
            <span className={cn(
              "flex-1 text-[13px] font-semibold",
              step.status === 'running' ? "text-[#3B82F6]" : "text-foreground"
            )}>
              {step.name}
            </span>
            {/* Duration */}
            {step.duration && (
              <span className="text-[11px] font-mono text-muted-foreground bg-muted px-2.5 py-1 rounded-lg">
                {step.duration}
              </span>
            )}
          </div>
      ...
      {(onViewLogs || status === 'failed') && (
        <div className="px-6 pb-6 pt-4 border-t border-border/60 flex gap-3">
          <button
            onClick={() => onViewLogs?.(id)}
            disabled={!onViewLogs}
            className={cn(
