...
const ICON_STYLE_MAP: Record<string, { bg: string; color: string }> = {
  CheckCircle2: { bg: 'bg-primary/10', color: 'text-primary' },
  Rocket: { bg: 'bg-blue-500/10', color: 'text-blue-600 dark:text-blue-400' },
  GitPullRequest: { bg: 'bg-red-500/10', color: 'text-red-500 dark:text-red-400' },
  MessageSquare: { bg: 'bg-purple-500/10', color: 'text-purple-600 dark:text-purple-400' },
  Box: { bg: 'bg-sky-500/10', color: 'text-sky-500 dark:text-sky-400' },
  Mail: { bg: 'bg-emerald-500/10', color: 'text-emerald-600 dark:text-emerald-400' },
};
                  ...
                  <AnimatePresence mode="popLayout">
                    ...
                    {!loading && filteredSkills.length === 0 && (
                      <motion.div
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        className="col-span-full flex flex-col items-center justify-center py-20 text-muted-foreground/80"
                      >
